...
func TestStreamingStress(t *testing.T) {
	...
	for i := range totalTasks { // process tasks lazily on the fly. memory footprint should remain low, stabilizing at ~10-20 MB for extremely high-frequency updates
		select {
		case <-ctx.Done():
			return
		default:
	...
	prog.Close() // workaround `go test` semantics; normally t.Cleanup(func() { prog.Close() }) should be called immediately after constructor
}
